...
<div class="modal fade" id="EditRoom{{ $data->id }}" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false"
            ...
            <form action="{{ route('room.update', $data->id) }}" method="POST">
              ...
              @csrf
                  ...
                  <div class="mb-3 text-start">
                      ...
                      <input type="text" class="form-control form-control-sm" id="" name="name" value="{{ $data->name }}"
                          placeholder="" required>
                  </div>
                  <div class="mb-3 text-start">
                    <label for="exampleFormControlTextarea1">รายละเอียด</label>
                    <textarea class="form-control" id="" name="detail" rows="3">{{ $data->detail }}</textarea>
                </div>
          </div>
